...
import gradio as gr
from func_utils import characterize_PSF, preprocess_images, characterize_SNR, simulate_training_data, train_localization_NN, test_NN, localize_all, one_click
...
def create_training_parameter_inputs():
...
def create_action_buttons(input_all, output_window):
    with gr.Row():
        ...
        button1.click(characterize_PSF, inputs=input_all, outputs=output_window)

        button2 = gr.Button("preprocess images")
        button2.click(preprocess_images, inputs=input_all, outputs=output_window)

        button3 = gr.Button("characterize SNR")
        button3.click(characterize_SNR, inputs=input_all, outputs=output_window)

    with gr.Row():
        button4 = gr.Button("simulate training data")
        button4.click(simulate_training_data, inputs=input_all, outputs=output_window)

        button5 = gr.Button("train a localization neural net")
        button5.click(train_localization_NN, inputs=input_all, outputs=output_window)

    with gr.Row():
        button6_0 = gr.Button("test the net")
        button6_0.click(test_NN, inputs=input_all, outputs=output_window)

        button6_1 = gr.Button("localize all")
        button6_1.click(localize_all, inputs=input_all, outputs=output_window)

    button7 = gr.Button("ONE CLICK", variant='huggingface')
    button7.click(one_click, inputs=input_all, outputs=output_window)
...
def build_demo():
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            blinking_inputs = create_training_parameter_inputs()
